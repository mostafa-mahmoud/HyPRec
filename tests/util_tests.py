...
class TestAbstractsPreprocessor(TestcaseBase):
    def runTest(self):
        ...
        self.assertEqual(max(self.abstracts_preprocessor.abstracts.keys()) + 1,
                         self.abstracts_preprocessor.get_num_items())
        self.assertTrue(isinstance(self.abstracts_preprocessor.get_term_frequency_sparse_matrix(), sparse.csr_matrix))
        self.assertEqual(max(map(lambda inp: len(inp.split(' ')), self.abstracts_preprocessor.abstracts.values())) + 1,
