...
class TestcaseBase(unittest.TestCase):
    def setUp(self):
        ...
        """
        self.documents, self.users = 18, 20
...
class TestEvaluator(TestcaseBase):
    def runTest(self):
        ...
        k_inverse = '%.2f' % numpy.round((1 / self.k_folds), 2)
        self.assertEqual(k_inverse, '%.2f' % numpy.round(numpy.count_nonzero(test1) / total_ratings, 2))

        self.assertEqual(k_inverse, '%.2f' % numpy.round(numpy.count_nonzero(test2) / total_ratings, 2))

        self.assertEqual(k_inverse, '%.2f' % numpy.round(numpy.count_nonzero(test3) / total_ratings, 2))
